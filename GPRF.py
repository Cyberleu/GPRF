...
class GPRF():
    def __init__(self, env_config, agent):
        self.env_config = env_config
        self.env = DataBaseEnv(self.env_config)
        self.agent = Agent()
    def run(self):
        for ep in range(self.env_config['train_args']['episodes']):
            total_reward = 0
            state = self.env.reset()
            while not self.env.done():
                mask = self.env.get_mask()
                action = self.agent.choose_action(state, mask)
