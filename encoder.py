...
class PlanEncoder(Encoder):
    def __init__(self, env):
        super(PlanEncoder ,self).__init__(env)
    # 返回矩阵和边信息，不是一个向量
...
class QueryEncoder(Encoder):
    def __init__(self, env):
        super(QueryEncoder ,self).__init__(env)
...
class SimpleEncoder(Encoder):
    ...
    def encode(self, gp):
        gp = GlobalPlan()
        vec = torch.empty(0).to(self.device)
        for plan in gp.singlePlans:
            vec = torch.cat(vec, self.encode_singleplan(plan))
        vec = torch.cat((vec, self.env.plans[self.env.plan_idx]))
